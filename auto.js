...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (cUser) {
      ...
      return res.status(400).json({
        error: true,
        message: "There's an issue on your appstate data."
      });
    }
  } catch (error) {
    return res.status(400).json({
      error: true,
      message: "There's an issue on you appstate data."
    });
  }
});
app.listen(3000, () => {
        ...
        var listenEmitter = api.listenMqtt(async (error, event) => {
          ...
          if (event.body && event.body?.toLowerCase().startsWith(prefix.toLowerCase()) && aliases(command)?.name) {
            if (blacklist.includes(event.senderID)) {
              api.sendMessage("Were very sorry you have been banned for some reason about using this bot, Please contact bot admin to immediately unbanned", event.threadID, event.messageID);
              return;
            }
          }
          if (event.body && aliases(command)?.name) {
